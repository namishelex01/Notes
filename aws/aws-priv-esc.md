...
- They can then login to the instance and request the associated AWS keys from the EC2 instance metadata, which gives them access to all the permissions that the associated instance profile/service role has.

**How to Exploit??**

Two ways to exploit:-

1. Create/Import an SSH key and associated it with the instance on creation, so they can SSH into it. 
2. Another way would be to supply a script in the EC2 User Data that would give them access, such as an Empire stager, or even just a reverse shell payload.
...
An example command to exploit this method might look like this:
	...
	aws ec2 run-instances \
		...
		–iam-instance-profile Name=iam-full-access-ip \
		–key-name my_ssh_key \
...
Not smart way as Guardduty wil catch it.
